...
| **1.18.0** | [`v1.18.0/`](v1.18.0/) | Post-perf-round sweep (6 subsystem auditors + a dedicated numeric/tensor/VM correctness hunt) after clearing 10 deferred debt items. 0 HIGH; C++ memory/UB/GC/race clean. 5 MED + 4 LOW fixed — all silent-failure/SSOT/docs issues: net gzip/deflate CRC-skip, Series.astype silent-String, DataFrame index non-validation, Set-read unhashable swallow, NaN Dict/Set key rejected, Tensor.round tie-break unified to scalar. | [`README.md`](v1.18.0/README.md) · [`FINDINGS.md`](v1.18.0/FINDINGS.md) · [`scan/`](v1.18.0/scan/) |
...
| ~~Dict / Set iteration order is not insertion-stable across delete~~ — **SUPERSEDED**: Dict/Set are now **INSERTION-ORDERED** | changed | The dense (Python-3.7-style) rewrite for card marking made Dict/Set iterate in insertion order, stable across delete (a delete tombstones, keeping surviving entries in place). `popitem`/`pop` are now LIFO. NaN keys stay write-only; the symmetric cross-type (BigInt/Integer) equality retry is preserved. Do NOT "restore" the old hash-order behaviour. |
