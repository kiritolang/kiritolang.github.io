...
int main() {
    ...
    CHECK(ev(vm, "String(int.from_string(\"354224848179261915075\"))") == "354224848179261915075");  // fib(100)
...
var i = 0
while i < 3000:
    var a = r.randbelow(2000000000) - 1000000000
    var b = r.randbelow(2000000000) - 1000000000
...
i = 0
while i < 500:
    var base = r.randbelow(61) - 30
    var e = r.randbelow(11)
...
for bs in range(2, 37):
    ...
    assert int.from_string(String(n)) == n
# isqrt floor property
i = 0
while i < 500:
    var x = r.randbelow(1000000000000000000)
    var s = int.isqrt(x)
...
i = 0
while i < 500:
    var m = 2 + r.randbelow(100000)
    var a2 = 1 + r.randbelow(m - 1)
...
for n in range(0, 2000):
    assert int.is_prime(n) == refprime(n)
    assert int.is_probable_prime(n) == refprime(n)
# random_prime output is prime with the requested bit length
for bits in [16, 32, 48, 64, 96]:
    var p = int.random_prime(bits)
    assert int.is_probable_prime(p)
...
"loops-ok")KI") == "loops-ok");
    ...
    CHECK(ev(vm, "String(int.from_string(\"ff\", 16))") == "255");
    CHECK(ev(vm, "String(int.from_string(\"zz\", 36))") == "1295");
    ...
    for (const char* c : {"561", "1105", "1729", "2465", "2821", "6601", "8911", "41041", "825265"}) {
        CHECK(ev(vm, std::string("int.is_probable_prime(") + c + ")") == "False");
        CHECK(ev(vm, std::string("int.is_prime(") + c + ")") == "False");
    }
    // Mersenne primes + large known primes recognised by Miller-Rabin.
    CHECK(ev(vm, "int.is_probable_prime(int.pow(2, 61) - 1)") == "True");   // M61
    CHECK(ev(vm, "int.is_probable_prime(int.pow(2, 89) - 1)") == "True");   // M89
    CHECK(ev(vm, "int.is_probable_prime(int.pow(2, 64) - 59)") == "True");  // largest 64-bit prime
    CHECK(ev(vm, "int.is_probable_prime(int.pow(2, 127) - 1)") == "True");  // M127

    // ---- BigInt methods ----
    CHECK(ev(vm, "int.pow(2, 100).bit_length()") == "101");
    CHECK(ev(vm, "int.BigInt(255).to_int()") == "255");
    CHECK(ev(vm, "type(int.BigInt(255).to_int())") == "Integer");
    CHECK_THROWS(ev(vm, "int.pow(2, 100).to_int()"));  // doesn't fit a native Integer
    ...
    CHECK_THROWS(ev(vm, "int.from_string(\"12x\", 10)"));   // bad digit
    CHECK_THROWS(ev(vm, "int.from_string(\"7\", 40)"));     // bad base
    ...
    CHECK(ev(vm, "int.is_prime(-7)") == "False");
    CHECK(ev(vm, "int.is_prime(0)") == "False");
    CHECK(ev(vm, "int.is_prime(1)") == "False");
