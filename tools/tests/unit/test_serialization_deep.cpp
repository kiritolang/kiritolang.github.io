...
int main() {
    ...
    CHECK(throws(vm, R"KI(import("serialize")
var x = 0
var i = 0
while i < 20000:
    x = [x]
...
serialize.dumps(x))KI"));
    CHECK(throws(vm, R"KI(import("dump")
var x = 0
var i = 0
while i < 20000:
    x = [x]
...
dump.dumps(x))KI"));
    ...
    CHECK(run(vm, R"KI(import("serialize")
var x = 0
var i = 0
while i < 50:
    x = [x]
...
ord(json.parse(r'"\r"')))KI") == "13");
    ...
    CHECK(run(vm, R"KI(import("json")
r"\b" in json.dumps("\b"))KI") == "True");
    CHECK(run(vm, R"KI(import("json")
r"\f" in json.dumps("\f"))KI") == "True");
    CHECK(run(vm, R"KI(import("json")
