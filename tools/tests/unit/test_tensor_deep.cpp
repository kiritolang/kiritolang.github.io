...
int main() {
    ...
    CHECK(run(vm, "var tensor = import(\"tensor\")\ntensor.tensor([1, 2, 3]).sum()") == "6.0");

    // ---- astype("Float") identity branch (Float -> Float, no cross to Complex) ----
    CHECK(run(vm, "var tensor = import(\"tensor\")\ntensor.ones([2]).astype(\"Float\").sum()") == "2.0");
...
z[0, 0] = complex.of(1, 2))KI"));
    ...
    CHECK(throws(vm, "var tensor = import(\"tensor\")\ntensor.tensor([-1.0, 4.0]).sqrt()"));
