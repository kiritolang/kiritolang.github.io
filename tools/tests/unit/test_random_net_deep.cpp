...
int main() {
    ...
    CHECK(run(vm, "import(\"random\")\nisinstance(Random(42).gauss(), Float)") == "True");
    CHECK(run(vm, "import(\"random\")\nisinstance(Random(42).gauss(5), Float)") == "True");
    CHECK(run(vm, "import(\"random\")\nisinstance(Random(42).expovariate(), Float)") == "True");
    // two identically-seeded generators agree (default gauss consumes the same draws)
    CHECK(run(vm, "import(\"random\")\nRandom(42).gauss() == Random(42).gauss()") == "True");

    // ---- under-arity guards throw ----
    CHECK(throws(vm, "import(\"random\")\nRandom(42).uniform(1)"));
    CHECK(throws(vm, "import(\"random\")\nRandom(42).sample([1, 2, 3])"));
    CHECK(throws(vm, "import(\"random\")\nRandom(42).randint(1)"));
    CHECK(throws(vm, "import(\"random\")\nRandom(42).seed()"));            // seed expects a value

    // ---- non-iterable / empty population guards throw ----
    CHECK(throws(vm, "import(\"random\")\nRandom(42).sample(5, 2)"));
    CHECK(throws(vm, "import(\"random\")\nRandom(42).choice(5)"));
    CHECK(throws(vm, "import(\"random\")\nRandom(42).choice([])"));

    // ---- resource / arity caps throw ----
    CHECK(throws(vm, "import(\"random\")\nRandom(42).choices([1, 2], k=300000000)"));  // k too large
    CHECK(throws(vm, "import(\"random\")\nRandom(42).randrange(1, 2, 3, 4)"));         // 1..3 args

    // ---- both engines: .generator name, and serialize round-trip reproduces the stream ----
    CHECK(run(vm, "import(\"random\")\nRandom(42, \"mersenne_twister\").generator") == "mersenne_twister");
    CHECK(run(vm, "import(\"random\")\nRandom(42).generator != \"mersenne_twister\"") == "True");  // default xoshiro
    CHECK(run(vm, R"KI(import("random")
import("serialize")
var r = Random(42)
...
r.randint(1, 1000000) == r2.randint(1, 1000000))KI") == "True");
    CHECK(run(vm, R"KI(import("random")
import("dump")
var r = Random(99, "mersenne_twister")
discard r.random()
